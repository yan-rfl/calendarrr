export type ParseResult =
  ...
  | { type: 'create_list'; events: Array<{ name: string; start_at: Date; detail?: string }> }
  | { type: 'list_today' }
...
export function parseLineMessage(text: string, now: Date = new Date(), tzOffsetMs = 0): ParseResult {
  ...
  if (lower.startsWith('/create-list')) {
    const lines = t.split('\n').slice(1).map(l => l.trim()).filter(Boolean)
    const events: Array<{ name: string; start_at: Date; detail?: string }> = []
    for (const line of lines) {
      const parsed = parseCreateBody(line, now, tzOffsetMs)
      if (parsed) events.push(parsed)
    }
    if (events.length > 0) return { type: 'create_list', events }
  }

  if (lower.startsWith('/create ')) {
    const body = t.slice(8).trim()
    const parsed = parseCreateBody(body, now, tzOffsetMs)
    if (parsed) return { type: 'create', ...parsed }
  }

  return { type: 'unknown', raw: t }
}

function parseCreateBody(body: string, now: Date, tzOffsetMs: number): { name: string; start_at: Date; detail?: string } | null {
  const structured = body.match(/^(.+)_(\d{4}-\d{2}-\d{2})\s+(\d{1,2}(?::\d{2})?)\s*(AM|PM)?(?:_(.+))?$/i)
  if (structured) {
  ...
  const todayAt = body.match(/^(.+)_Today at (\d{1,2}:\d{2})\s*(AM|PM)?$/i)
  if (todayAt) return { name: todayAt[1].trim(), start_at: parseLocalTime(todayAt[2], todayAt[3], now, 0, tzOffsetMs) }

  const tomorrowAt = body.match(/^(.+)_Tomorrow at (\d{1,2}:\d{2})\s*(AM|PM)?$/i)
  if (tomorrowAt) return { name: tomorrowAt[1].trim(), start_at: parseLocalTime(tomorrowAt[2], tomorrowAt[3], now, 1, tzOffsetMs) }

  const inRelative = body.match(/^(.+)_In (a|\d+)\s+(minutes?|hours?)$/i)
  if (inRelative) {
    ...
    return { name: inRelative[1].trim(), start_at: new Date(now.getTime() + ms) }
  }
